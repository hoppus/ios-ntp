...
- (NSTimeInterval) networkOffset {
  ...
  for (NetAssociation * timeAssociation in sortedArray) {
    ...
    //  NSLog(@"%@ timeAssociation %f", timeAssociation.server, timeAssociation.dispersion);
  ...
  NSLog(@"trustMe %i, usefulCount %i, timeAssociations.count %lu", self.trustMe, usefulCount, (unsigned long)timeAssociations.count );
  return timeInterval;
}
